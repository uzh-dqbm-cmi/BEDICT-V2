...
def get_datatensor_partitions(data_partitions, model_name, x_proto, y, ID, x_feat=None, 
                              fdtype=torch.float32, train_size=0.9, random_state=42):
    
    
    dpartitions = data_partitions
    ...
    if x_feat is not None:
        ...
        criscas_dtensor = ProtospacerDataset(x_proto, y,ID,  xproto_dtype, ydtype=ydtype)
...
def get_data_ready(args, normalize_opt = 'max', train_size=0.9, fdtype=torch.float32, plot_y_distrib=False):
    ## prepare the data
    ...
    x_protospacer,ID, y = data
    
    y = y.astype(np.float32)
    #print(y)
    ## transform to two dimentional vector
    #print('y  shape', y.shape)
    y_prob = np.vstack((y, 1-y))
    #print('y probe shape', y_prob.shape)
    y =np.transpose( y_prob)
    #y = y.astype(np.float32) 
...
'''
## for inference
def get_datatensor_partitions_for_inference(data_partitions, model_name, x_proto, y,ID,  x_feat=None, 
                              fdtype=torch.float32, train_size=0.9, random_state=42):
    train_val_test = {}
...
def prepare_sample_data(df,exp_name,efficiency):
    protospacer =  df[exp_name].apply(get_char)
    num_nucl = len(protospacer.columns)
    protospacer.columns = [f'B_encoded{i}' for  i in range(1, num_nucl+1)]
    protospacer.replace(['A', 'C', 'T', 'G'], [0,1,2,3], inplace=True)
    x_protospacer = np.array(protospacer)
    ...
    if efficiency in df.columns:
        ...
        y = np.zeros(x_protospacer.shape[0])
        print('true target is not provided, the correlation and loss will have no meaning')
        y = np.vstack((y, 1-y))
        y =np.transpose( y)
    ...
    ID = df['ID']
    return [x_protospacer, y,ID, x_non_protos_f]
