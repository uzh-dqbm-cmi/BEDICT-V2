...
#from models.data_process import generate_partition_datatensor
...
def perfmetric_report_regression(pred_target, ref_target, epoch, outlog):
    ...
    print('max(ref_target)',int(max(ref_target)))
    #print(pred_target.shape)
    if max(ref_target) != 0:
        print('we are here')
        ...
        spearman_corr = 'does not exist'
        pearson_corr = 'does not exist'
...
def build_regression_df( true_class, pred_class, seq_id):
    df_dict = {
            'id': seq_id,
